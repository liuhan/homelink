...
import com.smart.util.netty.Serial.kryo.KryoCodecUtil;
import com.smart.util.netty.Serial.kryo.KryoDecoder;
import com.smart.util.netty.Serial.kryo.KryoEncoder;
import com.smart.util.netty.Serial.kryo.KryoPoolFactory;
...
//        pipeline.addLast("decoder" , new KryoDecoder(util));
        ...
        // pipeline.addLast(new ReadTimeoutHandler(NettyConst.IM_NETTY_SERVER_READ_TIME_OUT));
        pipeline.addLast(new NettyHeartServerHandler());
        pipeline.addLast("handler", new NettyServerHandler(this.serverIP , this.serverPort ,this.serverHttpPort ,uuid));
